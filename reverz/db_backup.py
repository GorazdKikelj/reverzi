...
def download_from_s3(backup_s3_key, dest_file, manager_config):
    ...
    @param backup_s3_key - The key to use for the backup
    @param dest_file - The file to download to
    @param manager_config - The manager configuration to

    """
    s3_client = boto3.resource("s3")
    try:
        s3_client.meta.client.download_file(
            manager_config.get("AWS_BUCKET_NAME"), backup_s3_key, dest_file
        )
...
def backup_postgres_db(host, database_name, port, user, password, dest_file, verbose):
    ...
    """
    logger.info("Enter backup_postres_db")
    if verbose:
        ...
        try:
            ...
            if int(process.returncode) != 0:
                logger.debug(
                    "Command failed (1). Return code : {}".format(process.returncode)
                )
                print("Command failed. Return code : {}".format(process.returncode))
                #                exit(1)
                return False, f"Command failed (1). Return code : {process.returncode}"
        except Exception as e:
            logger.debug(f"Command failed (1). Return code : {e}")
            print(e)
            return False, e
        logger.debug(f"Backup completed: {output}")
        return True, output
    else:
        logger.debug("Silent run")
        try:
            process = subprocess.Popen(
                [
                    "/usr/bin/pg_dump",
            ...
            if process.returncode != 0:
                logger.debug(
                    "Command failed (2). Return code : {}".format(process.returncode)
                )
                print("Command failed (2). Return code : {}".format(process.returncode))
                return False, f"Backup command failed. Return code {process.returncode}"
            logger.debug(f"Backup completed: {output}")
            return True, output
        except Exception as e:
            logger.debug("Command failed (3). Return code : {e}")
            print(e)
            return False, e
...
def swap_after_restore(
    db_host, restore_database, new_active_database, db_port, user_name, user_password
):
    try:
        con = psycopg2.connect(
            dbname="postgres",
...
def move_to_local_storage(comp_file, filename_compressed, manager_config):
    ...
    shutil.move(
        comp_file,
        "{}{}".format(manager_config.get("LOCAL_BACKUP_PATH"), filename_compressed),
    )


def remove_file_from_local_storage(filename, manager_config):
    """Remove old dump file from /tmp directory"""
    try:
        os.remove(filename)
    #        os.remove('{}{}'.format(manager_config.get('BACKUP_PATH'),filename))
    except FileNotFoundError:
        pass


def backup_db(host, db, port, user, password, local_file_path):
    """
    Backup Postgres database to local file

    @param host - host where to connect to
    @param db - database name to be backed up
    ...
    @param local_file_path - path to local file where to store backup

    @return True if backup was successful False
    """
    timestr = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    filename = "backup-{}-{}.dump".format(timestr, db)
    filename_compressed = "{}.gz".format(filename)
    local_file = "{}{}".format(local_file_path, filename)
    result = backup_postgres_db(host, db, port, user, password, local_file, False)
    try:
        comp_file = compress_file(local_file)
    except Exception as e:
        return False, e
    ...
    manager_config = {
        ...
        "BACKUP_PATH": "/tmp/",
        "LOCAL_BACKUP_PATH": local_file_path,
    }
    try:
        move_to_local_storage(comp_file, filename_compressed, manager_config)
    except Exception as e:
        logger.debug(f"move_to_local_storage failed {e}")
        return False, e
    try:
        remove_file_from_local_storage(
            filename=local_file, manager_config=manager_config
        )
    except Exception as e:
        logger.debug(f"remove_file from_local_storage failed {e}")
        return False, e
...
def main():
    ...
    if args.action == "list":
        backup_objects = sorted(
            list_available_backups(storage_engine, manager_config), reverse=True
        )
    ...
    # backup task
        ...
        if "LOCAL" in storage_engine:
            ...
            remove_file_from_local_storage(
                filename=filename, manager_config=manager_config
            )
            logger.info(
                "Removed from {}{}".format(manager_config.get("BACKUP_PATH"), filename)
            )
    ...
    # restore task
        ...
        if not args.date:
            ...
            if storage_engine == "LOCAL":
                ...
                shutil.copy(
                    "{}/{}".format(
                        manager_config.get("LOCAL_BACKUP_PATH"), backup_match[0]
                    ),
                    restore_filename,
                )
